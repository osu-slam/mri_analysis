...
function searchlight_or_sr_clear(subj, study, dd)
%% Check input
...
for fold = blocks % for each fold... 
    ...
    or_set    = contains(theselabels, 'OR'); 
    sr_set    = contains(theselabels, 'SR');
    
    or_tst_set = block_tst_set & or_set & clear_set; 
    sr_tst_set = block_tst_set & sr_set & clear_set; 
    ...
    or_trn_set = block_trn_set & or_set & clear_set; 
    sr_trn_set = block_trn_set & sr_set & clear_set; 
    
%     repfactor = sum(or_trn_set)/sum(sr_trn_set);
...
%%% PARFOR START %%%
    parfor voxel_num = 1:numvox
        ...
        % Data
        or_trn_betas  = these_betas(or_trn_set, :); 
        sr_trn_betas  = these_betas(sr_trn_set, :); 
        Instances_trn = vertcat(or_trn_betas, sr_trn_betas); 
        
        or_tst_betas  = these_betas(or_tst_set, :); 
        sr_tst_betas  = these_betas(sr_tst_set, :); 
        Instances_tst = vertcat(or_tst_betas, sr_tst_betas); 
        
        % Labels
        Labels_trn_or =    ones(size(or_trn_betas, 1), 1);
        Labels_trn_sr = -1*ones(size(sr_trn_betas, 1), 1);
        Labels_tst_or =    ones(size(or_tst_betas, 1), 1);
        Labels_tst_sr = -1*ones(size(sr_tst_betas, 1), 1);

        Labels_trn = [Labels_trn_or; Labels_trn_sr];
        Labels_tst = [Labels_tst_or; Labels_tst_sr];
...
V.fname = fullfile(dir_data_MVPA, [subj.name '_' design.name '_beta_GNB_or_sr_clear_rad' num2str(radius) '.nii']);  
spm_write_vol(V, acc);
