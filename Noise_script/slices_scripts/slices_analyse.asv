...
tmp = spm_select(1,'.img','Select the first image of the first block')
...
for ses = 1:nses
  ...
  image_files_without_path = spm_select('List',pth,['^bold' num2str(ses) '.*\.img$']); 
  ...
  Pr = spm_select('Files',pth,['rbold',num2str(ses),'*.img']);
  Vr = spm_vol(Pr);
  
  Vo = spm_vol(spm_select('Files',pth,['meanbold',num2str(ses),'*.img']));
...
%        pth,filesep,'EPI',num2str(ses),'.hdr']) 
  ...
  Vm = spm_vol(spm_select('Files',pth,['mask',num2str(ses),'.img'])) 
 ...
 M = spm_vol(spm_select('Files',pth,['wmask',num2str(ses),'.img']));
