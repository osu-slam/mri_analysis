...
%if(~exist([templatedir,filesep,'xEPI.nii']))
%  disp(['Cannot find EPI template image in ',templatedir])
%  return
%end
%if(~exist([templatedir,filesep,'xwhole_brain.nii']))
...
for ses = 1:nses
  %P = spm_select('List',pth,['bold',num2str(ses),'*.nii']);
  image_files_without_path = spm_select('List',pth,['^bold' num2str(ses) '.*\.nii$']); 
  copies_of_directory_path = repmat(path,180,1); % Made a change here --Matt
...
%  Pr = spm_select('Files',pth,['rbold',num2str(ses),'*.nii']); % This
%  looked to be wrong, so I tried to fix it. --Matt 
  Pr = spm_select(Inf, 'any', 'Select realigned files', {}, pth, '.*', 1); 
  Vr = spm_vol(Pr);
 
  Po = spm_select(1, 'any', 'select meanbold', {}, pth, '.*', 1);
  Vo = spm_vol(Po); 
...
%  eval(['!cp ',templatedir,filesep,'xEPI.nii ',... 
%        pth,filesep,'EPI',num2str(ses),'.nii'])
