function snr_sd(study, subjs, runs)
...
%% Checks inputs
if (~ischar(study) || ~iscell(subjs) || ~iscell(runs))
    msg1  = 'Input must be snr_sd_v5(''study'', {subjects}, {runs}), where study is str, subjs and runs are cells'; 
    error(msg1);    
end
...
for s = 1:numsubjs % For each subject...
    ...
    try
        ...
        functional_dir = [subj_dir f 'FUNCTIONAL'];
        try
            cd(functional_dir)
        catch
            msg3=['I can''t seem to get to the subject''s FUNCTIONAL dir. Are you sure ' ...
              'this path is correct? ', functional_dir];
            error(msg3);
        end
    end
    ...
    % unprocessed files if none are found.
        ...
        % version two which I'm trying for sanity's sake
