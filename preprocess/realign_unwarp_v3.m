...
function realign_unwarp_v3(subj, study)
%% Check input
...
numscans = strsplit(numscans, '_'); numscans = numscans{end}; 
numscans = str2double(numscans);
...
filename = fullfile(dir_thissubj_batch, 'realign_unwarp_v3.mat');
...
for ii = 1:length(psfiles)
    target = fullfile(dir_motionps, ['realign_unwarp_v2_' num2str(ii) '.png']); 
